    ...
    public function get_control_output($controlEditingTypeModel)
    {
		...
		<div class="sb-control-toggle-set-ctn sbc-fb-fs"
			...
			<div
				...
				v-for="(toggle, toggleIndex) in control.options"
				role="radio"
				:aria-checked="<?php 
        ...
        ?>[control.id] == toggle.value"
			...
			>
				<div
					...
					role="button"
					tabindex="0"
					:aria-label="(control.heading || control.label || '') + ' (' + genericText.learnMore + ')'"
					v-show="shouldShowTogglesetCover(toggle)"
					@click.prevent.default="togglesetExtPopup(toggle)"
					@keydown.enter.prevent="togglesetExtPopup(toggle)"
					@keydown.space.prevent="togglesetExtPopup(toggle)"
				></div>
