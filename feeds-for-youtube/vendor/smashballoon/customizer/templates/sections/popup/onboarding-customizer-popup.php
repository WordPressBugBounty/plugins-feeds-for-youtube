...
<div v-for="tooltip in customizeScreensText.onboarding.tooltips" 
    ...
    role="dialog" aria-modal="true" v-bind:aria-labelledby="'sb-onboarding-tooltip-heading-' + customizeScreensText.onboarding.type + '-' + tooltip.step" tabindex="-1"
>
    <button type="button" class="sbc-popup-cls" aria-label="<?php esc_attr_e('Close', 'feeds-for-youtube'); ?>" @click.prevent.default="onboardingClose()">
    ...
    <div class="sb-onboarding-top-row">
		<p class="sb-standard-p sb-bold" v-bind:id="'sb-onboarding-tooltip-heading-' + customizeScreensText.onboarding.type + '-' + tooltip.step">{{tooltip.heading}}</p>
    ...
    <div class="sb-onboarding-bottom-row">
		...
		<div class="sb-previous-next-wrap">
			<div class="sbc-btn-wrapper">
				<button type="button" class="sbc-onboarding-previous sbc-hd-btn sbc-btn-grey sb-button-small sb-button-left-icon" v-bind:data-active="tooltip.step === 1 ? 'false' : 'true'" :disabled="tooltip.step === 1" @click.prevent.default="onboardingPrev">
				...
				<button type="button" v-if="customizeScreensText.onboarding.tooltips.length > tooltip.step" class="sbc-onboarding-next sbc-hd-btn sbc-btn-grey sb-button-small sb-button-right-icon" @click.prevent.default="onboardingNext">{{genericText.next}}
					<svg width="6" height="8" viewBox="0 0 6 8" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
						<path d="M1.66656 0L0.726562 0.94L3.7799 4L0.726562 7.06L1.66656 8L5.66656 4L1.66656 0Z" fill="#141B38"/>
					</svg>
				</button>
				<button type="button" v-if="customizeScreensText.onboarding.tooltips.length === tooltip.step" class="sbc-onboarding-finish sb-button-small ctf-btn-grey sbc-hd-btn" @click.prevent.default="onboardingClose">{{genericText.finish}}</button>
			</div>
		</div>
	</div>
</div>