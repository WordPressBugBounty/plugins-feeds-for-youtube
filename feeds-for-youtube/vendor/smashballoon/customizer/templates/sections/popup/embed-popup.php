<div class="sbc-embed-ctn sb-fs-boss sbc-center-boss" v-if="viewsActive.embedPopup">
	<div class="sbc-embed-popup sbc-popup-inside" role="dialog" aria-modal="true" :aria-label="embedPopupScreen.heading" tabindex="-1">
		<button type="button" class="sbc-popup-cls" aria-label="<?php esc_attr_e('Close', 'feeds-for-youtube'); ?>" @click.prevent.default="activateView('embedPopup')">
		...
		<h3 class="sbc-embed-step-heading" tabindex="-1" v-show="viewsActive.embedPopupScreen == 'step_1'">{{embedPopupScreen.heading}}</h3>
		...
		<div class="sbc-embed-step-1 sbc-fs" v-show="viewsActive.embedPopupScreen == 'step_1'">
			<div class="sbc-embed-step-1-top sbc-fs">
				...
				<div class="sbc-embed-input-ctn sbc-fs">
					<input class="sbc-fs" type="text" readonly :aria-label="genericText.shortcodeText || 'Shortcode'" :value="'[youtube-feed feed='+ customizerFeedData.feed_info.id +']'">
			...
			<div class="sbc-embed-step-1-bottom sbc-fs">
				...
				<div class="sbc-embed-btns-ctn sbc-fs">
					<button type="button" class="sbc-embed-btn sbc-btn-grey" @click.prevent.default="switchScreen('embedPopupScreen','step_2')">
		...
		<div class="sbc-embed-step-2 sbc-fs" v-show="viewsActive.embedPopupScreen == 'step_2'">
			<div class="sb-embed-breadcrumb sbc-fs">
                <svg width="6" height="8" viewBox="0 0 6 8" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><path d="M5.27203 0.94L4.33203 0L0.332031 4L4.33203 8L5.27203 7.06L2.2187 4L5.27203 0.94Z" fill="#434960"></path></svg>
                <button type="button" class="sb-embed-breadcrumb-link" @click.prevent.default="switchScreen('embedPopupScreen','step_1')">{{embedPopupScreen.heading}}</button>
			</div>
			<h3 class="sbc-embed-step-heading" tabindex="-1">{{embedPopupScreen.addPage}}</h3>
			<div class="sbc-embed-step-2-list">
				...
				<div class="sbc-embed-step-2-pages sbc-fs">
					<div class="sb-control-toggle-set-ctn sbc-fs">
						<div class="sb-control-toggle-elm sbc-fs sb-tr-2" role="button" tabindex="0" v-for="page in wordpressPageLists" :aria-pressed="viewsActive.embedPopupSelectedPage == page.id ? 'true' : 'false'" :data-active="viewsActive.embedPopupSelectedPage == page.id" @click.prevent.default="switchScreen('embedPopupSelectedPage',page.id)" @keydown.enter.prevent.default="switchScreen('embedPopupSelectedPage',page.id)" @keydown.space.prevent.default="switchScreen('embedPopupSelectedPage',page.id)">
