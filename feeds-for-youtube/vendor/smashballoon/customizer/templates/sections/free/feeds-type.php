<div class="sbc-feedtypes-ctn sbc-fs sb-box-shadow" v-if="viewsActive.selectedFeedSection == 'feedsType' && !iscustomizerScreen">
	<div class="sbc-feedtypes sbc-fs">
		...
		<div class="sbc-feedtypes-list">
			<button type="button" class="sbc-feedtype-el" v-for="(feedTypeEl, feedTypeIn) in feedTypes" :aria-pressed="selectedFeed == feedTypeEl.type && feedTypeEl.type != 'socialwall' ? 'true' : 'false'" :data-active="selectedFeed == feedTypeEl.type && feedTypeEl.type != 'socialwall'" :data-type="feedTypeEl.type" @click.prevent.default="chooseFeedType(feedTypeEl)">
