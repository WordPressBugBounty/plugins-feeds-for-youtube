<div v-if="viewsActive.pageScreen == 'selectFeed' && !iscustomizerScreen && !fullScreenLoader" class="sbc-fs">
	<div class="sbc-create-ctn sbc-fs">
		<div class="sbc-feedtype-heading">
			<h1 class="sbc-creation-step-heading" tabindex="-1" v-if="viewsActive.selectedFeedSection !== 'selectSource'">{{selectFeedTypeScreen.mainHeading}}</h1>
			<h1 class="sbc-creation-step-heading" tabindex="-1" v-if="viewsActive.selectedFeedSection == 'selectSource'">{{selectFeedTypeScreen.addSource}}</h1>

			<button type="button" class="sbc-btn sbc-slctf-nxt sbc-btn-ac sbc-btn-orange" @click.prevent.default="creationProcessNext()">
	...
	<div class="sbc-ft-action sbc-slctfd-action sbc-fs">
		<div class="sbc-wrapper">
			<button type="button" class="sbc-slctf-back sbc-hd-btn sbc-btn-grey" @click.default.prevent="creationProcessBack()">
				<svg width="7" height="11" viewBox="0 0 7 11" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
