...
class Sender {
	...
	public function send( array $payload ) {
		...
		if ( '' === Config::get_api_url() ) {
			// Constant undefined or blanked via the sby_smash_usage_tracking_api_url
		...
		$max_bytes = (int) apply_filters( 'sby_smash_usage_tracking_max_payload_bytes', Config::MAX_PAYLOAD_BYTES );
		if ( $max_bytes > 0 && strlen( $body ) > $max_bytes ) {
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG && defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG && function_exists( 'error_log' ) ) {
				error_log( '[SBY Usage Tracking] Payload size ' . strlen( $body ) . ' exceeds max ' . $max_bytes . ', send skipped.' );
			}
			return 0;
		}

		$timeout = (int) apply_filters( 'sby_smash_usage_tracking_request_timeout', Config::REQUEST_TIMEOUT );
		...
		$response = wp_remote_post(
			...
			array(
				...
				'body'        => $body,
				'user-agent'  => 'SBY/' . (defined( 'SBYVER' ) ? SBYVER : '') . '; ' . get_bloginfo( 'url' ),
			)
		);
