<?php
/**
 * Records usage events for the current period.
 *
...
class EventRecorder {
	...
	const OPTION_NAME = 'sby_smash_usage_events';

	/**
	 * Error counter categories tracked for the API error breakdown.
	 *
	...
	/**
	 * Increment the counter for an API error category.
	 *
	 * Categories are fixed (see self::$error_categories); anything unknown is
	 * folded into 'other' so the stored map always has the same shape.
	 *
	 * @param string $category One of quota, rate_limit, auth, permission, not_found, server, network, other.
	 */
	public static function increment_error_counter( $category ) {
		if ( ! Config::is_enabled() ) {
			return;
		}
		if ( ! is_string( $category ) || ! in_array( $category, self::$error_categories, true ) ) {
			$category = 'other';
		}

		$counters = get_option( Config::OPTION_ERROR_COUNTERS, array() );
		if ( ! is_array( $counters ) ) {
			$counters = array();
		}

		$normalized = array();
		foreach ( self::$error_categories as $key ) {
			$normalized[ $key ] = isset( $counters[ $key ] ) ? (int) $counters[ $key ] : 0;
		}
		++$normalized[ $category ];

		update_option( Config::OPTION_ERROR_COUNTERS, $normalized, false );
	}
}
